...
class _Report {
  const _Report(this.name, this.description);
  final String name;
...
const _groups = [
  _ReportGroup('SALES & RECEIVABLES', [
    _Report('Sales register', 'Invoice-level with tax split and margin'),
    _Report('Customer aging', '0–30 / 31–60 / 61–90 / 90+ buckets'),
    _Report('Customer statement', 'Printable account statement per customer'),
    _Report('Item-wise sales', 'Quantity and value by SKU, category or brand'),
  ]),
  _ReportGroup('PURCHASING & PAYABLES', [
    _Report('Purchase register', 'PO to invoice with receipt variance'),
    _Report('Supplier aging', 'Open payables by due bucket'),
    _Report('Pending receipts', 'Ordered vs received quantities'),
    _Report('Price variance', 'PO rate against invoice rate'),
  ]),
  _ReportGroup('INVENTORY', [
    _Report('Stock summary', 'On hand, reserved and available by warehouse'),
    _Report('Stock movement', 'Full in/out ledger per SKU'),
    _Report('Valuation report', 'Moving average and closing value'),
    _Report('Batch & expiry', 'Ageing batches and near-expiry stock'),
  ]),
  _ReportGroup('FINANCE & TAX', [
    _Report('Trial balance', 'Any date, any level of the account tree'),
    _Report('Profit & loss', 'Period comparison with variance'),
    _Report('Balance sheet', 'As at date, schedule-wise'),
    _Report('Cash & bank book', 'Daily balances and reconciliation'),
    _Report('GSTR-1 / 3B', 'Return-ready summaries and JSON export'),
  ...
  _ReportGroup('PEOPLE', [
    _Report('Attendance register', 'Daily and monthly muster'),
    _Report('Payroll register', 'Gross to net with statutory heads'),
    _Report('Leave balance', 'Accrued, used and encashable'),
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      'One reporting framework · every report accepts date, company, branch and warehouse filters',
  ...
  Widget build(BuildContext context) {
    return MouseRegion(
      ...
      child: SectionCard(
        ...
        child: Container(
          decoration: BoxDecoration(border: Border.all(color: _hover ? AppColors.accent : Colors.transparent)),
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            mainAxisSize: MainAxisSize.min,
            children: [
              Text(widget.report.name, style: AppText.sans(size: 12.5, weight: FontWeight.w600)),
